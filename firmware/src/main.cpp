...
// Persisted across deep-sleep cycles but lost on full reboot. Used to send
// If-None-Match so the bridge can 304 us when the frame hasn't changed.
RTC_DATA_ATTR static char rtcEtag[24] = {0};
...
static bool triggerAdvance(const String& bridgeUrl) {
  // Derive the /advance URL from the /frame URL.
  String url = bridgeUrl;
  int slash = url.lastIndexOf('/');
  if (slash >= 0) url = url.substring(0, slash) + "/advance";

  HTTPClient http;
  http.setTimeout(HTTP_TIMEOUT_MS);
  if (!http.begin(url)) {
    Serial.println("advance: begin failed");
    return false;
  }
  int code = http.POST("");
  Serial.printf("advance: POST %s -> %d\n", url.c_str(), code);
  http.end();
  // Forget the cached etag so the follow-up /frame won't return 304 against
...
// Derive the /data/all bundle URL from a /frame or /data URL.
static String bundleUrlFor(const String& bridgeUrl) {
  int slash = bridgeUrl.lastIndexOf('/');
  String base = slash < 0 ? bridgeUrl : bridgeUrl.substring(0, slash);
  return base + "/data/all";
}
...
static bool fetchAndCacheBundle(const String& bridgeUrl) {
  const String url = bundleUrlFor(bridgeUrl);
  HTTPClient http;
  http.setTimeout(HTTP_TIMEOUT_MS);
  if (!http.begin(url)) {
    Serial.println("bundle: http begin failed");
    return false;
  }
  http.addHeader("Accept", "application/octet-stream");
  // Piggyback the battery reading on the bundle request — the bridge keeps
  // a rolling 24h in-memory history and synthesizes a battery panel from it.
  ...
  if (contentLen <= 8 || contentLen > 32 * 1024) {
    Serial.printf("bundle: bad length %d\n", contentLen);
    http.end();
    return false;
  }
  uint8_t* buf = (uint8_t*)malloc(contentLen);
  if (!buf) {
    Serial.printf("bundle: malloc(%d) failed\n", contentLen);
    http.end();
    return false;
  }
  ...
  while ((int)got < contentLen && http.connected()) {
    if (stream->available()) {
      int n = stream->readBytes(buf + got, contentLen - got);
      if (n > 0) { got += n; lastProgress = millis(); }
    } else if (millis() - lastProgress > 5000) {
      Serial.println("bundle: stalled");
      break;
    } else {
      delay(1);
    }
  ...
  if ((int)got != contentLen) {
    Serial.printf("bundle: short read %u/%d\n", got, contentLen);
    free(buf);
    return false;
  }
  ...
  if (magic != BUNDLE_MAGIC || version != BUNDLE_VERSION_REQUIRED || panelCount == 0) {
    Serial.printf("bundle: bad header magic=0x%04x ver=%u count=%u\n",
  ...
  if (!prefs.begin("x3-cache", false)) {
    Serial.println("bundle: NVS open failed");
    free(buf);
    return false;
  }
  // Drop the old blob explicitly before writing the new one. NVS can't hold
  // two copies of a multi-KB blob at once in our partition, so update-in-place
  // putBytes can silently fail. Explicit remove + put is the supported path.
  prefs.remove(kBundleKey);
  size_t wrote = prefs.putBytes(kBundleKey, buf, contentLen);
  prefs.putUInt("total", panelCount);
  size_t freeEntries = prefs.freeEntries();
  prefs.end();
  free(buf);
  Serial.printf("bundle: putBytes wrote=%u/%d, NVS freeEntries=%u\n",
                (unsigned)wrote, contentLen, (unsigned)freeEntries);
  if (wrote != (size_t)contentLen) {
    Serial.printf("bundle: NVS write FAILED — keeping prior cache state\n");
    return false;
  }
  rtcPanelTotal = panelCount;
  Serial.printf("bundle: cached %d bytes, %u panels\n", contentLen, panelCount);
  return true;
}
...
void setup() {
  ...
  String password = prefs.getString("wifi_pass", DEFAULT_WIFI_PASSWORD);
  String bridgeUrl = prefs.getString("bridge_url", DEFAULT_BRIDGE_URL);
  prefs.end();
  ...
  if (!buttonWake && !quiet) {
    if (rtcLastFetchEpoch == 0 || !haveValidClock) {
      ...
      Serial.printf("refresh: %llds since last fetch (interval=%llus view=%s) → fetch=%d\n",
                    (long long)elapsed, (unsigned long long)currentRefreshInterval(),
                    viewLabel(rtcViewMode), (int)needFetch);
    }
  } else if (quiet) {
  ...
  bool fetchOk = false;
  if (needFetch) {
    // Only paint the splash on first boot (nothing else to show). Background
    // refreshes leave the current panel on screen — the user doesn't need to
    // know we're talking to the bridge.
    if (rtcLastFetchEpoch == 0) {
      paintConnectingScreen(ssid, rtcCycleCount);
    }
    wifiOk = connectWifi(ssid, password);
    if (wifiOk) {
      // The bridge response's HTTP Date header sets the clock — no NTP
      // roundtrip needed.
      fetchOk = fetchAndCacheBundle(bridgeUrl);
      if (fetchOk) {
  ...
  // Sleep duration: aim for the next 15-min fetch boundary. On button wake
  // we sleep the residual time; on a successful fetch wake we sleep a full
  // interval. Floor at 60s so we don't thrash on tiny intervals.
  ...
  if (haveValidClock && inQuietHours()) {
    ...
    // Never had a successful fetch (first boot or bridge wasn't ready) —
    // retry quickly rather than waiting a full refresh interval.
