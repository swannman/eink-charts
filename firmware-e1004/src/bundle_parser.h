#pragma once
// Reader for the reTerminal E1004 "color dashboard bundle" (magic 0xCFB2,
// version 3) produced by bridge-cloud/.../data_trmnl.py via push_e1004. Same
// wire structure as the TRMNL X's v2 bundle, but every shade byte carries an
// E Ink Spectra 6 color code (see config.h SPECTRA_*) instead of a 0..15 gray.
// Bounds-checked little-endian cursor plus a few bundle-level navigation
// helpers. The renderer streams panels straight out of the cached blob with
// BinReader, so there's no big intermediate model.
#include <stddef.h>
#include <stdint.h>
#include <string.h>

constexpr uint16_t TRMNL_BUNDLE_MAGIC = 0xCFB2;
constexpr uint8_t TRMNL_BUNDLE_VERSION = 3;   // v3: shade bytes are Spectra 6 colors
