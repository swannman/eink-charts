...
static constexpr const char* NS = "trmnlx-crypto";
...
bool exists() {
  ...
  bool ok = (p.getBytesLength(K_SK) == KEY_LEN) && (p.getBytesLength(K_PK) == KEY_LEN);
  p.end();
  return ok;
}
...
bool generate_and_store(uint8_t sk_out[KEY_LEN], uint8_t pk_out[KEY_LEN]) {
  ...
  int ret;
  do {
    static const char pers[] = "trmnlx-x25519-keygen";
    ...
    size_t pk_olen = 0;
    ret = mbedtls_ecp_point_write_binary(&grp, &pk_point, MBEDTLS_ECP_PF_UNCOMPRESSED,
    ...
    Preferences p;
    if (!p.begin(NS, /*readOnly=*/false)) { Log.println("keystore: NVS open failed"); break; }
    p.remove(K_SK);
    p.remove(K_PK);
